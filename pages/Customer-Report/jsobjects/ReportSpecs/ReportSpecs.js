export default {
	...
	visibleFieldOptions: [
		...
		{ group: "Vendor / Account", value: "vendor", label: "Vendor", description: "Vendor name — the customer's pretty name where one is set, otherwise the global pretty name, otherwise the vendor's plain name", sql: "COALESCE((SELECT NULLIF(btrim(cvpn.pretty_name), '') FROM bill_management_v2.customers_vendors_pretty_name cvpn JOIN bill_management_v2.vendors v ON v.id = cvpn.vendor_id WHERE v.code = amf.vendor_code AND cvpn.customer_id = amf.customer_id LIMIT 1), (SELECT NULLIF(btrim(MIN(v.pretty_name)), '') FROM bill_management_v2.vendors v WHERE v.code = amf.vendor_code HAVING COUNT(DISTINCT btrim(v.pretty_name)) = 1), (SELECT NULLIF(btrim(cpv.name), '') FROM bill_management_v2.customers_providers_vendors cpv WHERE cpv.code = amf.vendor_code AND cpv.customer_id = amf.customer_id LIMIT 1), (SELECT NULLIF(btrim(MIN(pv.name)), '') FROM bill_management_v2.providers_vendors pv WHERE pv.code = amf.vendor_code HAVING COUNT(DISTINCT btrim(pv.name)) = 1), amf.vendor_code) AS \"vendor\"" },

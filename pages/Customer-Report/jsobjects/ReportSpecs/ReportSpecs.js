export default {
	...
	visibleFieldOptions: [
		...
		{ group: "Totals", measure: true, value: "costPerSqft", label: "Cost per SqFt", description: "The group's total cost divided by the site's floor area. Source: locations.square_feet, read as a maximum so the figure is right whether or not Square Feet is shown as a column of its own. Blank where UBM holds no area for the site.", sql: "ROUND(SUM(amf.total_charges) / NULLIF(MAX(l.square_feet), 0), 4) AS \"costPerSqft\"", sqlExcl: "ROUND((SUM(amf.total_charges) - COALESCE(SUM(ob.otc / NULLIF(obn.feed_rows, 0)), 0)) / NULLIF(MAX(l.square_feet), 0), 4) AS \"costPerSqft\"" },
		...
		{ group: "Totals", measure: true, value: "kbtuPerSqft", label: "KBTUs per SqFt", description: "KBTUs divided by the site's floor area — energy use intensity. Same factors and same area source as Cost per SqFt. Blank where UBM holds no area for the site.", sql: "ROUND(SUM(amf.total_consumption * CASE upper(btrim(amf.utility_type)) WHEN 'ELECTRIC' THEN 3412 WHEN 'LIGHTING' THEN 3412 WHEN 'SOLARPV' THEN 3412 WHEN 'NATURALGAS' THEN 100000 WHEN 'UTILITYGAS' THEN 100000 WHEN 'PROPANE' THEN 91452 WHEN 'OIL2' THEN 138500 WHEN 'DIESEL' THEN 137381 WHEN 'GASOLINE' THEN 120214 WHEN 'STEAM' THEN 1000 ELSE 0 END) / 1000 / NULLIF(MAX(l.square_feet), 0), 4) AS \"kbtuPerSqft\"" },
		{ group: "Totals", measure: true, value: "usagePerSqft", label: "Usage per SqFt", description: "The group's total usage divided by the site's floor area, in whatever unit the group is measured in. Same area source as Cost per SqFt.", sql: "ROUND(SUM(amf.total_consumption) / NULLIF(MAX(l.square_feet), 0), 4) AS \"usagePerSqft\"" },
